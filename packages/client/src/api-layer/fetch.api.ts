...
const api = {
  ...
  post: <ReqBody, TResponse>(url: string, reqBody: ReqBody) =>
    request<TResponse>(url, { ...reqInit, method: "POST", body: JSON.stringify(reqBody) }),
  ...
  put: <ReqBody, TResponse>(url: string, reqBody: ReqBody) =>
    request<TResponse>(url, { ...reqInit, method: "PUT", body: JSON.stringify(reqBody) }),
};
